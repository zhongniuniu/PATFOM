...
from PAT_projector import PAT3DProjector
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    PAT_projector = PAT3DProjector([1, 1024, 1024], [512, 800], 64)
    PAT_projector_128 = PAT3DProjector([1, 1024, 1024], [512, 800], 256)

    test(model, args, epoch, PAT_projector)


def test(model, args, epoch, PAT_projector):
    ...
    if not os.path.exists(join("/public2/zhongyutian/PATFOM-main/Test_output/infer/", epoch_str)):
        os.mkdir(join("/public2/zhongyutian/PATFOM-main/Test_output/infer/", epoch_str))
    if not os.path.exists(join("/public2/zhongyutian/PATFOM-main/Test_output/label/", epoch_str)):
        os.mkdir(join("/public2/zhongyutian/PATFOM-main/Test_output/label/", epoch_str))
    ...
    for key in test_keys:
        ...
        with torch.no_grad():
            for i, tup in enumerate(data_loader):
                ...
                ultra_image = label / torch.max(label)  # [B, C, H, W], [0, 1]
                ultra_image = ultra_image.permute(1, 2, 3, 0)  # [C, H, W, 1]

                ultra_image = ultra_image.unsqueeze(0)
                ultra_image = ultra_image.cuda()

                ultra_projs = PAT_projector.forward_project(
                    ultra_image.transpose(1, 4).squeeze(1))  # [bs, x, y, z] -> [bs, n, h, w]
                fbp_recon = PAT_projector.backward_project(ultra_projs)  # [bs, n, h, w] -> [bs, x, y, z]
                # print(fbp_recon.shape) #torch.Size([1, 1024, 1024, 1])
                fbp_recon = fbp_recon.unsqueeze(1).transpose(1, 4)
                # print(fbp_recon.shape)
                slice_idx = [0]

                fbp_recon = fbp_recon[0, slice_idx, ...].permute(0, 3, 1, 2).cpu().data  # [c, 1, h, w]

                fbp_recon = fbp_recon.repeat(1, 3, 1, 1)

                fbp_recon_maks = fbp_recon
                fbp_recon_maks = fbp_recon_maks.cuda()

                mask = model(fbp_recon_maks)

                save_dir1 = "/public2/zhongyutian/PATFOM-main/Test_output/infer"
