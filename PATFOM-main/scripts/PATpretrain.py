...
def train(train_loader, model, optimizer, scheduler, epoch, args, writer, loss_list):
    ...
    for i, tup in enumerate(train_loader):
        ...
        # 应用随机多个平均掩膜
        img = apply_random_average_mask_pytorch(img.cpu(), block_size, mask_ratio)
        ...
        loss = L1_LOSS(pred, label) + 10 * ffl(pred, label)
        ...
        if i % args.print_freq == 0:
            ...
            # print('L1:[{0}] Per:[{1}] ffl:[{2}]\t'.format(L1_LOSS(pred, label), perceptual_loss(pred1, label1), 10 * ffl(pred, label)))
