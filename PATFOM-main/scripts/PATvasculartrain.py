...
def train(train_loader, model, optimizer, scheduler, epoch, args, writer,loss_list):
    ...
    for i, tup in enumerate(train_loader):
        ...
        vgg = models.vgg16(pretrained=True).features[:16].cuda().eval() #4 2 8best
        vgg1 = models.vgg16(pretrained=True).features.cuda().eval()
        selected_layers = [1, 6, 11, 18]
        weights = [1.0, 0.75, 1, 0.2] # 可调

        style_loss = 0
        for i, layer in enumerate(selected_layers):
            test_feat = vgg1[:layer + 1](pred1)
            style_feat = vgg1[:layer + 1](label1)
            G_test = gram(test_feat)
            G_style = gram(style_feat)
            style_loss += weights[i] * loss_mse(G_test, G_style)

        # Calculate features using VGG image classifiers
        # ref_features = vgg(img1)
        test_features = vgg(pred1)
        input_features = vgg(img1)

        # get the MSE Loss function

        # What layer within the CNN to calculate the content loss. Low layers (0, 1...) have low
        # level features like lines. High layers have object classifications like 'eye' or 'car'
        # content_feature = 1
        L1_LOSS = nn.L1Loss()
        perceptual_loss_fn = PerceptualLoss()
        # calculate the content loss
        content_loss = loss_mse(input_features, test_features) #ref_features test_features

        style_features = vgg(label1)

        # print(style_features.shape)

        # Get the gram matrices
        style_gram = gram(style_features)
        test_gram = gram(test_features)


        output_edges = sobel_edge_detection(pred1)
        content_edges = sobel_edge_detection(img1)
        edge_loss = loss_mse(output_edges, content_edges)

        loss = content_loss + 1500000 * style_loss
...
def gram(x):
    ...
    (bs, ch, h, w) = x.size()
    f = x.view(bs, ch, w * h)
    G = f.bmm(f.transpose(1, 2)) / (ch * h * w) #(ch * h * w) or (h * w) or None
    return G
