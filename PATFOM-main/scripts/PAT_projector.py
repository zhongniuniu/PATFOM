...
class Initialization_ConeBeam:
    def __init__(self, image_size, num_proj, start_angle, proj_size, raw_reso=0.7):
        ...
        ## Imaging object (reconstruction objective) with object center as origin 以对象中心为原点的成像对象(重建对象)
        ...
        ## Detector
        self.param['sh'] = self.param['sx']*(2000/1000)
        self.param['sw'] = np.sqrt(self.param['sx']**2+self.param['sy']**2)*(1500/1000)
        self.param['nh'] = proj_size[0] # shape of sinogram正投结果 is proj_size*proj_size
        self.param['nw'] = proj_size[1]
        self.param['dde'] = 500*self.reso # distance between origin and detector center (assume in x axis)原点到探测器中心的距离(假设在x轴上)
        self.param['dso'] = 1500*self.reso # distance between origin and source (assume in x axis)原点与源的距离(假设在x轴上)
