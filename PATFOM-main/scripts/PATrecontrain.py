...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_list = []
        # train for one epoch
        train(train_loader, model, optimizer, scheduler, epoch, args, writer, PAT_projector, PAT_projector_128, loss_list)
...
def train(train_loader, model, optimizer, scheduler, epoch, args, writer, PAT_projector, PAT_projector_128, loss_list):
    ...
    for i, tup in enumerate(train_loader):
        ...
        projs = PAT_projector_128.forward_project(ultra_image.transpose(1, 4).squeeze(1))  # [bs, x, y, z] -> [bs, n, h, w]
        # print(projs.shape)
        sample_step = projs.size(1) // 64
        ultra_projs = projs[:, :64*sample_step:sample_step, :, :].cuda()   # 从第0个维度开始，每隔6个取一个
        ...
        # print(fbp_recon_maks.shape)

        optimizer.zero_grad()
        # compute output
        pred = model(fbp_recon_maks)


        L1_LOSS = nn.L1Loss()
        perceptual_loss_fn = PerceptualLoss()

        label = F.interpolate(label, size=(400, 400), mode='bilinear', align_corners=False)
        ...
        loss = 10 * L1_LOSS(pred, label) + perceptual_loss_fn(pred1, label1)
