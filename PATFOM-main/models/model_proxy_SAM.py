...
from models.segment_anything.modeling.image_encoder import Block
from models.segment_anything import sam_model_registry
from models.encoder.setup_sam import setup_model as build_sam

from models.loss_functions import dice_loss, multilabel_dice_loss
from models.utils import init_weights


class MSConv2d(nn.Module):
    def __init__(self, ch, groups=1):
...
class PATSAM(nn.Module):
    def __init__(self, model_name, checkpoint, num_classes=1,
                 is_finetune_image_encoder=False,
                 use_adaptation=False,
                 adaptation_type='learnable_prompt_layer',
                 head_type='custom',
                 reduction=4, upsample_times=2, groups=1, rank=4) -> None:
        super(PATSAM, self).__init__()
        
        #load same from the pretrained model
        if model_name == 'PAT':
            self.sam = build_sam(checkpoint=checkpoint, num_multimask_outputs=num_classes)
        ...
        if self.head_type == 'custom':
            ...
            for i in range(upsample_times):
                self.up_conv["up_{}".format(i+1)] = nn.Sequential(
                        nn.ConvTranspose2d(dim, dim, 1, 1),
                        LayerNorm2d(dim),
                        nn.GELU()
                    )
                dim = dim
