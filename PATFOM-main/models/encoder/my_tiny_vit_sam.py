...
class SuperResolutionEncoder(nn.Module):
    def __init__(self):
        super(SuperResolutionEncoder, self).__init__()
        self.encoder = nn.Sequential(
            # 输入: [1, 3, 300, 300]
            nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1),  # 输出: [1, 64, 150, 150]
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),

            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),  # 输出: [1, 128, 75, 75]
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),

            nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1),  # 输出: [1, 256, 38, 38]
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),

            nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1),  # 输出: [1, 256, 19, 19]
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),

            nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1),  # 输出: [1, 256, 10, 10]
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),

            nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1),  # 输出: [1, 256, 5, 5]
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),

            # 在最终层不使用BatchNorm以避免尺寸过小导致的问题
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=0),  # 输出: [1, 256, 3, 3]
            nn.ReLU(inplace=True),

            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=0),  # 输出: [1, 256, 1, 1]
            nn.ReLU(inplace=True)
        )
...
class TinyViT(nn.Module):
    def __init__(self, img_size=1024, in_chans=3, num_classes=256,
    ...
    def forward_features(self, x):
        ...
        x = self.layers[0](x)
        # print(x.shape) #torch.Size([1, 16384, 128])
    ...
    def forward(self, x):
        x = self.forward_features(x).cuda(non_blocking=True)
