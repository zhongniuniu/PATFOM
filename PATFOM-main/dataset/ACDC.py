...
class AcdcDataset(Dataset):
    def __init__(self, keys, args, mode='train'):
        ...
        print(f'dataset length: {len(self.files)}') #106
    ...
    def __getitem__(self, index):
        ...
        # img = np.stack((img,) * 3, axis=-1) #是否为三通道使用
        img = np.asarray(img).transpose([2, 0, 1])


        img, label = self.transform(img, label) #废弃这里的label

        label1 = Image.open(self.files[index].replace('imgs/', 'annotations/'))
        preprocess = transforms.Compose([
            transforms.Resize((512, 512)),  # 调整大小为跟img一致大小 1024
    ...
    def transform(self, img, label):
        ...
        if self.mode == 'train':
            aug_list = [  # CenterCropTransform(crop_size=target_size),
                ...
                # GammaTransform(p_per_sample=0.5),
                # GaussianNoiseTransform(p_per_sample=0.5),
                ResizeTransform(target_size=self.patch_size),  # resize
                # MirrorTransform(axes=(1,)),
                # SpatialTransform(patch_size=self.patch_size, random_crop=False,
                #                  patch_center_dist_from_border=self.patch_size[0] // 2,
                #                  do_elastic_deform=True, alpha=(100., 350.), sigma=(40., 60.),
            ...
            # print('val transform')
            aug_list = [
                # ResizeTransform(target_size=self.patch_size),
                NumpyToTensor(),
            ]
            aug = Compose(aug_list)
